    ...
    if total_vectors == 0:
        ...
        for barcode_dir in barcode_folders:
            ...
            if not vector_file:
                continue
            data = np.load(vector_file[0], allow_pickle=True)
            vec_data = data['vectors']
            n = vec_data.shape[0]
            combined_vectors[idx:idx+n, :] = vec_data
        ...
        # Fit-transform in chunks to avoid RAM overload
        batch_size = 50000  # adjust according to available RAM
        ...
        for start in range(0, total_vectors, batch_size):
            ...
            embeddings[start:end] = umap_model.fit_transform(combined_vectors[start:end])
            log_message(f"Processed vectors {start} to {end}")

        # ---------------------------
        # HDBSCAN clustering
        # ---------------------------
        log_message("Running HDBSCAN clustering...")
        ...
        scatter = plt.scatter(embeddings[:,0], embeddings[:,1], c=labels, cmap='tab20', s=5, alpha=0.7)
        ...
        for cluster_id in unique_labels:
            ...
            plt.text(centroid[0], centroid[1], str(cluster_id), fontsize=12, fontweight='bold',
