...
identity_threshold = 75  # Adjusted to 75%
...
def blast_with_timeout(seq, timeout=300):
    """
    Run NCBIWWW.qblast in a thread with timeout.
    User can press Enter to skip.
    Returns result_handle or None if skipped/timed out.
    """
    q = queue.Queue()

    def target():
        try:
            result = NCBIWWW.qblast("blastn", "nt", seq, hitlist_size=1)
            q.put(result)
    ...
    print(f"  ⏳ BLAST started, timeout {timeout}s. Press ENTER to skip...")

    for _ in range(timeout * 10):
        if not thread.is_alive():
            break
        if check_enter_pressed():
            print("  ⏩ Skipping BLAST on user request.")
            return None
        time.sleep(0.1)
...
fungal_keywords = [
    ...
    "Peziza", "Malassezia", "Rhizopus", "Mucor", "Pyronema", "Penicillium", "Henningsomyces",
    "Trichocladium", "zygomycete", "Serendipita", "Pseudogymnoascus"
]
...
with open(fasta_blast_output_path, "w") as fasta_out, \
    ...
    for record in SeqIO.parse(fasta_in, "fasta"):
        ...
        try:
            result_handle = blast_with_timeout(seq, timeout=calculated_timeout)
        except Exception as e:
        ...
        time.sleep(3)  # Respect NCBI usage policy
